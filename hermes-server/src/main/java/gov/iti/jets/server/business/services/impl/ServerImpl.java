...
public class ServerImpl extends UnicastRemoteObject implements Server {
    ...
    public void addPrivateChat(PrivateGroupDetailsDto privateGroupDetailsDto) throws RemoteException {
        ...
        int groupId = privateGroupService.addNewPrivateGroupChat(privateGroupDetailsDto);
        if(groupId==-1)
            return;
        Client sender = connectedClients.get(privateGroupDetailsDto.firstContact);
        Client receiver = connectedClients.get(privateGroupDetailsDto.secondContact);
        String senderName = DaosFactory.INSTANCE.getUserDao().getUserByPhone(privateGroupDetailsDto.firstContact).get().name;
        String receiverName = DaosFactory.INSTANCE.getUserDao().getUserByPhone(privateGroupDetailsDto.secondContact).get().name;

        GroupDto groupDtoToSender = new GroupDto(groupId,receiverName,getUserProfilePicture(privateGroupDetailsDto.secondContact),receiver.getOnlineStatus());
        groupDtoToSender.status = receiver.getOnlineStatus();

        GroupDto groupDtoToReceiver = new GroupDto(groupDtoToSender.id,senderName,getUserProfilePicture(privateGroupDetailsDto.firstContact),sender.getOnlineStatus());

        sender.addNewContactOnInvitationAcceptance(groupDtoToSender);
        receiver.addNewContactOnInvitationAcceptance(groupDtoToReceiver);
    }
