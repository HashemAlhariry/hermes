...
public class InvitationServiceImpl implements InvitationService {
	...
	public void sendInvitation(InvitationSentDto invitationSentDto, Map<String, Client> connectedClients) {
		...
		invitationSentDto.invitedPhones.forEach(clientPhone -> {
			...
			try {
				...
				// check all testcases passed and check if already invitation send for a private
				// chat before
				if (casesChecked
						&& DaosFactory.INSTANCE.getInvitationDao().checkInvitationAvailability(new InvitationEntity(0,
								invitationSentDto.senderPhone, clientPhone, InvitationStatus.PENDING))) {
					...
					// save invitation to db when user leave it the notification menu and make it
					// pending
					DaosFactory.INSTANCE.getInvitationDao().createInvitation(new InvitationEntity(0,
							invitationSentDto.senderPhone, clientPhone, InvitationStatus.PENDING));
				}
	...
	public void updatingInvitation(InvitationResponseDto invitationResponseDto) {
		...
		DaosFactory.INSTANCE.getInvitationDao().updateInvitationStatus(new InvitationEntity(0,
				invitationResponseDto.senderPhone, invitationResponseDto.receiverPhone, invitationStatus));
